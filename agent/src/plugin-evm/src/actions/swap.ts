...
import { swapTemplate } from "../templates/index.ts";
import type { SwapParams, Transaction } from "../types/index.ts";
import { getChainConfigs, WalletProvider } from "../providers/wallet.ts";
...
export class SwapAction {
  ...
  constructor(private walletProvider: WalletProvider) {
    this.config = createConfig({
      integrator: "eliza",
      chains: Object.values(getChainConfigs(this.walletProvider.runtime)).map(
        (config) => ({
          id: config.chainId,
          name: config.name,
  ...
  async swap(params: SwapParams): Promise<Transaction> {
    ...
    const routes = await getRoutes({
      fromChainId: getChainConfigs(this.walletProvider.runtime)[params.chain]
        .chainId as ChainId,
      toChainId: getChainConfigs(this.walletProvider.runtime)[params.chain]
        .chainId as ChainId,
